...
void printTrem(Vagao trem[], int tam) {
	...
	for (i = 0; i < tam; i++) {
		if (trem[i].tipo == 'P'){
			printf("[%d, %s] ", trem[i].carga.passageiros.numeroDePassageiros, trem[i].carga.passageiros.classe);
		} else {
			printf("[%s %.2f] ", trem[i].carga.mercadoria.mercadoriaTransportada, trem[i].carga.mercadoria.pesoEmToneladas);
		}
	}
	printf("\n");
}
